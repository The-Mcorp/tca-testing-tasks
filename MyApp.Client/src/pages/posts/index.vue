  ...
  <div class="relative bg-gray-50 dark:bg-gray-900 px-6 pt-16 pb-20 lg:px-8 lg:pt-24 lg:pb-28">
    ...
    <div class="relative my-4 mx-auto max-w-7xl">
      <div class="flex flex-wrap justify-center">
        <RouterLink v-for="tag in allTags" :key="tag" :to="tagLink(tag)" class="mr-2 mb-2 text-xs leading-5 font-semibold bg-slate-400/10 dark:bg-slate-400/30 rounded-full py-1 px-3 flex items-center space-x-2 hover:bg-slate-400/20 dark:hover:bg-slate-400/40 dark:highlight-white/5">{{tag}}</RouterLink>
      </div>
    </div>
    <div class="relative mb-8 mx-auto max-w-7xl">
      <div class="flex flex-wrap justify-center">
        <b class="text-sm font-semibold">{{ thisYear }}</b>
        <RouterLink v-for="year in allYears.filter(x => x != thisYear)" :key="year" class="ml-3 text-sm text-indigo-700 dark:text-indigo-300 font-semibold hover:underline" :to="yearLink(year)">{{year}}</RouterLink>
      </div>
    </div>
    <div class="relative mx-auto max-w-7xl">
